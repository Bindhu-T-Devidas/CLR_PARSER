...
st.markdown("Upload a grammar file with productions (e.g., `E->E+T`) one per line. Use `#` to represent epsilon (ε).")
    ...
    for entry in table.values():
        for val in entry.values():
            if isinstance(val, str) and ',' in val:
                actions = val.split(',')
                r = sum(1 for x in actions if x.strip().startswith("r"))
                s = sum(1 for x in actions if x.strip().startswith("s"))
                if r and s:
                    sr += 1
                elif r > 1:
                    rr += 1
